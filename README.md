...
1. 克隆仓库：
   ```
   git clone https://github.com/your-username/OrangeHRM-Test-Framework.git
...
1. 配置仓库 Settings → Actions → General → 启用 Workflow permissions（Read and write）。

2. push/PR 时自动触发测试 + Allure 结果 artifact。

3. 查看 CI 状态：仓库 Actions 标签。
...
* 报告示例：
  测试用例 | 状态 | 持续时间 | 细节
  --- | --- | --- | ---
  test_valid_credentials | PASSED | 10s | 成功登录 Dashboard
  test_invalid_password | FAILED | 8s | Invalid credentials 提示
